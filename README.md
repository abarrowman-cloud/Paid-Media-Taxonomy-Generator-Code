...
- **Every platform emits a bare ID**, LinkedIn included. LinkedIn URLs carry a
  URN type (`activity`, `ugcPost`, `share`); the type is used to find the number
  and to reject non-post namespaces, then discarded. The row's own `platform`
  column qualifies the ID downstream.
